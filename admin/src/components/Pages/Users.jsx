...
const Users = () => {
  ...
  const apiEndpoint = `http://localhost:8084/api/v1/users`;
  ...
  useEffect(() => {
    let nonAdmins = data.filter((user) => user.isAdmin === "false");
    // let response = nonAdmins.filter((user) => user.isAdmin === "false");
    setAllAdmins(nonAdmins);
  }, [data]);
  ...
  const ModifyUser = async (action, id, token) => {
    try {
      ...
      await axios.put(
        `http://localhost:8084/api/v1/users/Modify/${action}`,
        body,
        config
      );
